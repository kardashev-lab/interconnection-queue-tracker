    ...
    <header className="relative overflow-hidden">
      <div className="pointer-events-none absolute inset-0">
        <Image
          ...
          sizes="100vw"
        />
        <div className="absolute inset-0 bg-gradient-to-r from-white from-[30%] via-white/85 to-white/35 sm:from-[34%] sm:via-white/80 sm:to-white/25" />
        <div className="absolute inset-0 bg-gradient-to-t from-[#f7f6f3] via-white/30 to-white/20" />
      </div>
        ...
        <div className="flex flex-col gap-8">
          <div className="flex flex-col gap-8 lg:flex-row lg:items-stretch lg:gap-10">
            <div className="flex max-w-xl flex-1 flex-col justify-center lg:py-2">
              <h1 className="font-display text-4xl text-[#2f3437] sm:text-5xl">
...
function HeroStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex min-h-[5.5rem] flex-col items-center justify-center px-5 py-4 text-center sm:min-h-[6.25rem] sm:px-6">
      ...
      <p className="mt-1.5 font-metric text-3xl text-[#2f3437]">{value}</p>
    </div>
  );
}
